...
<link rel="stylesheet" href="__PUBLIC__/statics/sui-0.6.1/css/sm-extend.min.css">
php;
    return $link;
    }
...
<script src='__PUBLIC__/statics/sui-0.6.1/js/sm-extend.min.js'></script>
php;
    return $link;
    }
...
php;
     ...
     * word：        提示文字
     * multiple：    是否开启多文件选择
     * checkedbool  是否添加radio
     * goonuploader：是否增加继续上传按钮   
     */
    public function _webuploader($tag){
        $url=isset($tag['url'])?$tag['url']:U('Home/Index/ajax_upload');
        $name=isset($tag['name'])?$tag['name']:'file_name';
        $word=isset($tag['word'])?$tag['word']:'<i style="color:red;">选择一张为主图</i>';
        $multiple = isset($tag['multiple'])?$tag['multiple']:true;
        $goonuploader = isset($tag['goonuploader'])?$tag['goonuploader']:true;
        $checkedbool = isset($tag['checkedbool'])?$tag['checkedbool']:true;
