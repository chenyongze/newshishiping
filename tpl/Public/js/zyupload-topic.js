...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
	    ...
	    //         	html += '				</div>';
		   ...
		   //          html += '			</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">上传图片</div>';
		            // html += '					<div class="upload_btn">开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview">';
				    html += '				<div class="add_upload"></div>';
					html += '			</div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">确认上传文件</button>';
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type, para.fileType) > -1){
							...
							if (file.size >= para.fileSize) {
								ie.alert('您这个"'+ file.name +'"文件大小过大');
							} else {
								// 在这里需要判断当前所有文件中
								arrFiles.push(file);	
							}
						}else{
							ie.alert('您这个"'+ file.name +'"上传类型不符合');
						}
					}else{
						ie.alert('您这个"'+ file.name +'"没有类型, 无法识别');	
					}
				}
				return arrFiles;
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if(para.del){  // 显示删除按钮
					delHtml = '<span class="file_del" data-index="'+file.index+'" title="删除"><i class="file_mark"></i></span>';
				}
...
//								}
			...
			this.addEvent = function(){
				...
				$(".upload_btn").bind("click", function(e){
					var push_title = $('#input-title').val();
					var push_content = $('#input-textarea').val();
					if(push_title == ''){
						ie.alert("标题不能为空！");
						return;
					}
